...
<div class="breadcrumbwidget">
    <ul class="breadcrumb">
        ...
        <li class="active">Danh sách <?=$config['product'][$type]['title_main_capbon']?></li>
    </ul>
</div>
<!-- End Bảng Điều Khiển -->

<div class="pagetitle">
	<h1><?=$config['product'][$type]['title_main_capbon']?></h1>
</div>
	...
	<div class="contentinner">
		<div class="row-fluid">
			<div class="span12">
				...
				<form action="index.php?com=product&act=man_capbon&type=<?=$_GET['type']?>" method="POST" name="update_position">
					<table class="table table-bordered" <?=(count($items))?'id="dyntable"':''?>>
						<thead>
							<tr>
								...
								<th>STT</th>
								<?php if($config['product'][$type]['show_images_capbon']=='true') { ?>
						        	<th>Hình đại diện</th>
						    	<?php } ?>
								<th>Tên danh mục</th>
						       	<th>Hiển thị</th>
						...
						<tr><input type="hidden" name="stt[id][]" value="<?=$items[$i]["id"]?>">
							...
							<td style="width:5%;"><input type="number" min="0" name="stt[stt][]" id="stt" value="<?=$items[$i]['stt']?>" data-id="<?=$items[$i]['id']?>" data-table="product_capbon" class="selectinput update_stt" /></td>
							<?php if($config['product'][$type]['show_images_capbon']=='true') { ?>
								<td style="width:3%;">
									<a href="index.php?com=product&act=edit_capbon&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id=<?=$items[$i]['id']?>&type=<?=$_GET['type']?>" title="Chỉnh sửa"><img style="max-width: 60px;max-height: 60px;margin: auto;" onerror="src='img/noimage.png'" src="<?=_upload_product?><?=$items[$i]['photo']?>" alt="<?=$items[$i]['tenvi']?>"></a>
								</td>
							<?php } ?>
							<td style="width:25%;" class="text-left">
								<a href="index.php?com=product&act=edit_capbon&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id=<?=$items[$i]['id']?>&type=<?=$_GET['type']?>" title="Chỉnh sửa"><?=$items[$i]['tenvi']?></a>
							</td>
							<td style="width:15%;" class="action">
								<a data-table="product_capbon" data-id="<?=$items[$i]['id']?>" data-loai="hienthi" class="show_ajax" title="Hiển thị">
							...
							<td style="width:10%;" class="action">
								<a href="index.php?com=product&act=edit_capbon&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&id=<?=$items[$i]['id']?>&type=<?=$_GET['type']?>" title="Chỉnh sửa"><span class="iconfa-edit"></span></a>&nbsp&nbsp
								<a href="index.php?com=product&act=delete_capbon&id=<?=$items[$i]['id']?>&type=<?=$_GET['type']?>" title="Xóa" onClick="if(!confirm('Bạn có chắc muốn xóa mục này?')) return false;"><span class="iconfa-trash"></span></a>
							</td>
						</tr>
				...
				<div class="actionfull">
					<a href="index.php?com=product&act=add_capbon&type=<?=$_GET['type']?>" class="btn btn-rounded"><icon class="icon-plus"></icon> Tạo mới</a>
...
<script>
	...
	function onSearch(evt){	
		var keyword = document.getElementById("keyword").value;		
			location.href = "index.php?com=product&act=man_capbon&type=<?=$_GET['type']?>&keyword="+keyword;
	...
	if(jQuery('#delall').length > 0) {
		jQuery('#delall').click(function(){
			...
			if (hoi==true) document.location = "index.php?com=product&act=delete_capbon&type=<?=$_GET['type']?>&listid=" + listid;
		});
	}
	...
	function select_onchange_1()
	{
		...
		var url = "index.php?com=product&act=man_capbon&type=<?=$_GET['type']?>";
