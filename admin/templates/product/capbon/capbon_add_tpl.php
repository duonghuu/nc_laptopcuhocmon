...
<div class="breadcrumbwidget">
    <ul class="breadcrumb">
        ...
        <li class="active">Chi tiết <?=$config['product'][$type]['title_main_capbon']?></li>
    </ul>
</div>
<!-- End Bảng Điều Khiển -->

<div class="pagetitle">
	<h1><?php if ($_REQUEST['act']=="edit_capbon") echo "Cập nhật"; else echo "Thêm mới"; ?></h1>
</div>
		...
		<div class="row-fluid">
			<div class="span12">
				...
				<div class="widgetcontent bordered shadowed nopadding">
					<form name="frm" method="post" action="index.php?com=product&act=save_capbon&type=<?=$_GET['type']?>" enctype="multipart/form-data" class="stdform stdform2">
					    ...
					    <?php if($config['product'][$type]['images_capbon']=='true') { ?>
						    <?php if($_GET['act']=='edit_capbon') { ?>
						    ...
						    <?php } ?>
						    <p><label>Upload hình ảnh:</label><span class="field"><input type="file" name="file" class="uniform-file" /> <strong><?php echo $config['product'][$type]['img_type_capbon']." - Width: ".$config['product'][$type]['width_capbon']." px - Height: ".$config['product'][$type]['height_capbon']." px"?></strong></span></p>
						<?php } ?>
                        ...
                        <div id="tabs">
                            ...
                            <?php foreach($config['lang'] as $key => $value) { ?>
                                <div id="tabs-<?=$key?>">
                                    ...
                                    <?php if($config['product'][$type]['mota_capbon']=='true') { ?>
                                        ...
                                        <?php if($config['product'][$type]['mota_cke_capbon']=='true') { ?>
                                            <script type="text/javascript">
                                                var mota<?=$key?> = CKEDITOR.replace('mota<?=$key?>');
                                            </script>
                                        <?php } ?>
                                    <?php } ?>
                                    <?php if($config['product'][$type]['noidung_capbon']=='true') { ?>
                                        ...
                                        <?php if($config['product'][$type]['noidung_cke_capbon']=='true') { ?>
                                            <script type="text/javascript">
                        ...
                        <?php if($config['product'][$type]['seo_capbon']=='true') { ?>
					    <!-- Thông Tin SEO -->
						...
						<p class="stdformbutton">
							...
							<button type="button" onclick="javascript:window.location='index.php?com=product&act=man_capbon&type=<?=$_GET['type']?>'" class="btn"><span class="iconfa-off"></span> Thoát</button>
						</p>	
					</form>
				</div>
			</div>
...
<script type="text/javascript">
	...
	{				
		...
		var url = "index.php?com=product&act=<?php if($_REQUEST['act']=='edit_capbon') echo 'edit_capbon'; else echo 'add_capbon';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&type=<?=$_GET['type']?>";
