<?php
	function get_main_list()
	{
		...
		$str='<select id="id_list" name="id_list" class="chzn-select">
...
<div class="breadcrumbwidget">
    <ul class="breadcrumb">
        ...
        <li class="active">Chi tiết <?=$config['product'][$type]['title_main_cat']?></li>
    </ul>
</div>
<!-- End Bảng Điều Khiển -->

<div class="pagetitle">
	<h1><?php if ($_REQUEST['act']=="edit_cat") echo "Cập nhật"; else echo "Thêm mới"; ?></h1>
</div>
		...
		<div class="row-fluid">
			<div class="span12">
				...
				<div class="widgetcontent bordered shadowed nopadding">
					<form name="frm" method="post" action="index.php?com=product&act=save_cat&type=<?=$_GET['type']?>" enctype="multipart/form-data" class="stdform stdform2">
					    <p><label>Danh mục cấp 1:</label><span class="field"><?=get_main_list();?></span></p>
			
                        <?php if($config['product'][$type]['images_cat']=='true') { ?>
    					    ...
    					    <?php } ?>
    					    <p><label>Upload hình ảnh:</label><span class="field"><input type="file" name="file" class="uniform-file" /> <strong><?php echo $config['product'][$type]['img_type_cat']." - Width: ".$config['product'][$type]['width_cat']." px - Height: ".$config['product'][$type]['height_cat']." px"?></strong></span></p>
                        <?php } ?>
                        ...
                        <div id="tabs">
                            ...
                            <?php foreach($config['lang'] as $key => $value) { ?>
                                <div id="tabs-<?=$key?>">
                                    ...
                                    <?php if($config['product'][$type]['mota_cat']=='true') { ?>
                                        ...
                                        <?php if($config['product'][$type]['mota_cke_cat']=='true') { ?>
                                            <script type="text/javascript">
                                                var mota<?=$key?> = CKEDITOR.replace('mota<?=$key?>');
                                            </script>
                                        <?php } ?>
                                    <?php } ?>
                                    <?php if($config['product'][$type]['noidung_cat']=='true') { ?>
                                        ...
                                        <?php if($config['product'][$type]['noidung_cke_cat']=='true') { ?>
                                            <script type="text/javascript">
                        ...
                        <?php if($config['product'][$type]['seo_cat']=='true') { ?>
                        <!-- Thông Tin SEO -->
						...
						<p class="stdformbutton">
							...
							<button type="button" onclick="javascript:window.location='index.php?com=product&act=man_cat&type=<?=$_GET['type']?>'" class="btn"><span class="iconfa-off"></span> Thoát</button>
						</p>	
					</form>
				</div>
			</div>
