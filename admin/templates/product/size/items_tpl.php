...
<div class="pagetitle">
	<h1>Danh sách size <?=$config['product'][$type]['title_main']?></h1>
</div>
...
<div class="maincontent">
	<div class="contentinner">
		<div class="row-fluid">
			<div class="span12">
				<form action="index.php?com=product&act=man_size&type=<?=$_GET['type']?>" method="POST" name="update_position">
					<table class="table table-bordered" <?=(count($items))?'id="dyntable"':''?>>
						...
						<tr><input type="hidden" name="stt[id][]" value="<?=$items[$i]["id"]?>">
							...
							<td style="width:5%;"><input type="number" min="0" name="stt[stt][]" id="stt" value="<?=$items[$i]['stt']?>" data-id="<?=$items[$i]['id']?>" data-table="product_size" class="selectinput update_stt" /></td>
							<td style="width:10%;">
								<a href="index.php?com=product&act=edit_size&type=<?=$_GET['type']?>&id=<?=$items[$i]['id']?>" title="Cập nhật"><?=$items[$i]['tenvi']?></a>
							</td>
							<td style="width:5%;" class="action">
								<a data-table="product_size" data-id="<?=$items[$i]['id']?>" data-loai="hienthi" class="show_ajax" title="Hiển thị">
							...
							<td style="width:5%;" class="action">
								<a href="index.php?com=product&act=edit_size&type=<?=$_GET['type']?>&id=<?=$items[$i]['id']?>" title="Sửa"><span class="iconfa-edit"></span></a>&nbsp
								<a href="index.php?com=product&act=delete_size&type=<?=$_GET['type']?>&id=<?=$items[$i]['id']?>" onClick="if(!confirm('Bạn có chắc muốn xóa dữ liệu ?')) return false;" title="Xóa"><span class="iconfa-trash"></span></a>
							</td>
						</tr>
				...
				<div class="actionfull">
					<a href="index.php?com=product&act=add_size&type=<?=$_GET['type']?>" class="btn btn-rounded"><icon class="icon-plus"></icon> Thêm mới</a>
...
<script>
	...
	if(jQuery('#delall').length > 0) {
		jQuery('#delall').click(function(){
			...
			if (hoi==true) document.location = "index.php?com=product&act=delete_size&type=<?=$_GET['type']?>&listid=" + listid;
		});
	}
