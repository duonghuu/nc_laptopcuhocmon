...
<div class="breadcrumbwidget">
    <ul class="breadcrumb">
        ...
        <li class="active">Chi tiết <?=$config['product'][$type]['title_main_list']?></li>
    </ul>
</div>
<!-- End Bảng Điều Khiển -->
		...
		<div class="row-fluid">
			<div class="span12">
				...
				<div class="widgetcontent bordered shadowed nopadding">
					<form name="frm" method="post" action="index.php?com=product&act=save_list&type=<?=$_GET['type']?>" enctype="multipart/form-data" class="stdform stdform2">
						
                        <?php if($config['product'][$type]['images_list']=='true') { ?>
                            ...
                            <?php } ?>
                            <p><label>Upload hình ảnh:</label><span class="field"><input type="file" name="file" class="uniform-file" /> <strong><?php echo $config['product'][$type]['img_type_list']." - Width: ".$config['product'][$type]['width_list']." px - Height: ".$config['product'][$type]['height_list']." px"?></strong></span></p>
                        <?php } ?>

                        <!-- Kiểm tra nhiều hình ảnh -->
                        <?php if(count($config['product'][$type]['multipic_list_arr'])>0) { foreach ($config['product'][$type]['multipic_list_arr'] as $key => $value) {
                        ...
                        <?php if($config['product'][$type]['mausac_list']=='true') { ?>
                            <!-- ColorPicker -->
                        ...
                        <div id="tabs">
                            ...
                            <?php foreach($config['lang'] as $key => $value) { ?>
                                <div id="tabs-<?=$key?>">
                                    ...
                                    <?php if($config['product'][$type]['mota_list']=='true') { ?>
                                        ...
                                        <?php if($config['product'][$type]['mota_cke_list']=='true') { ?>
                                            <script type="text/javascript">
                                                var mota<?=$key?> = CKEDITOR.replace('mota<?=$key?>');
                                            </script>
                                        <?php } ?>
                                    <?php } ?>
                                    <?php if($config['product'][$type]['noidung_list']=='true') { ?>
                                        ...
                                        <?php if($config['product'][$type]['noidung_cke_list']=='true') { ?>
                                            <script type="text/javascript">
                        ...
                        <?php if($config['product'][$type]['seo_list']=='true') { ?>
					    <!-- Thông Tin SEO -->
						...
						<p class="stdformbutton">
							...
							<button type="button" onclick="javascript:window.location='index.php?com=product&act=man_list&type=<?=$_GET['type']?>'" class="btn"><span class="iconfa-off"></span> Thoát</button>
						</p>	
					</form>
				</div>
			</div>
