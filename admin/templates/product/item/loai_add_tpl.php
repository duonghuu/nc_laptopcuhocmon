<?php
	...
	function get_main_cat()
	{
		...
		$stmt=mysql_query($sql);
		$str='
		<select id="id_cat" name="id_cat" class="chzn-select my-chzn-select">
...
<div class="breadcrumbwidget">
    <ul class="breadcrumb">
        ...
        <li class="active">Chi tiết <?=$config['product'][$type]['title_main_item']?></li>
    </ul>
</div>
<!-- End Bảng Điều Khiển -->
		...
		<div class="row-fluid">
			<div class="span12">
				...
				<div class="widgetcontent bordered shadowed nopadding">
					<form name="frm" method="post" action="index.php?com=product&act=save_item&type=<?=$_GET['type']?>" enctype="multipart/form-data" class="stdform stdform2">
					    ...
					    <?php if($config['product'][$type]['images_item']=='true') { ?>
						    ...
						    <?php } ?>
						    <p><label>Upload hình ảnh:</label><span class="field"><input type="file" name="file" class="uniform-file" /> <strong><?php echo $config['product'][$type]['img_type_item']." - Width: ".$config['product'][$type]['width_item']." px - Height: ".$config['product'][$type]['height_item']." px"?></strong></span></p>
						<?php } ?>
                        ...
                        <div id="tabs">
                            ...
                            <?php foreach($config['lang'] as $key => $value) { ?>
                                <div id="tabs-<?=$key?>">
                                    ...
                                    <?php if($config['product'][$type]['mota_item']=='true') { ?>
                                        ...
                                        <?php if($config['product'][$type]['mota_cke_item']=='true') { ?>
                                            <script type="text/javascript">
                                                var mota<?=$key?> = CKEDITOR.replace('mota<?=$key?>');
                                            </script>
                                        <?php } ?>
                                    <?php } ?>
                                    <?php if($config['product'][$type]['noidung_item']=='true') { ?>
                                        ...
                                        <?php if($config['product'][$type]['noidung_cke_item']=='true') { ?>
                                            <script type="text/javascript">
                        ...
                        <?php if($config['product'][$type]['seo_item']=='true') { ?>
					    <!-- Thông Tin SEO -->
						...
						<p class="stdformbutton">
							...
							<button type="button" onclick="javascript:window.location='index.php?com=product&act=man_item&type=<?=$_GET['type']?>'" class="btn"><span class="iconfa-off"></span> Thoát</button>
						</p>	
					</form>
				</div>
			</div>
...
<script type="text/javascript">
	...
	{				
		...
		var url = "index.php?com=product&act=<?php if($_REQUEST['act']=='edit_item') echo 'edit_item'; else echo 'add_item';?><?php if($_REQUEST['id']!='') echo"&id=".$_REQUEST['id']; ?>&type=<?=$_GET['type']?>";
