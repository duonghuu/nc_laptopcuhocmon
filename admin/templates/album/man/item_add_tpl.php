...
<div class="breadcrumbwidget">
    <ul class="breadcrumb">
        ...
        <li class="active">Chi tiết <?=$config['album'][$type]['title_main']?></li>
    </ul>
</div>
<!-- End Bảng Điều Khiển -->
		...
		<div class="row-fluid">
			<div class="span12">
				...
				<div class="widgetcontent bordered shadowed nopadding">
					<form name="frm" method="post" action="index.php?com=album&act=save&type=<?=$_GET['type']?>" enctype="multipart/form-data" class="stdform stdform2">
					    
                        <?php if($config['album'][$type]['images']=='true') { ?>
                            ...
                            <?php } ?>
                            <p><label>Upload hình ảnh:</label><span class="field"><input type="file" name="file" class="uniform-file" /> <strong><?php echo $config['album'][$type]['img_type']." - Width: ".$config['album'][$type]['width']." px - Height: ".$config['album'][$type]['height']." px"?></strong></span></p>
                        <?php } ?>
                                    ...
                                    <div class="content-multi-pic">
                                        <?php for($i=0;$i<count($list_hinhanh);$i++) { ?>
                                            <div class="item_pic item_trash_pic<?=$list_hinhanh[$i]['id']?>">
                                                <img class="img_item_pic" width="100" src="<?=_upload_photo.$list_hinhanh[$i]['photo']?>" />
                        ...
                        <div id="tabs">
                            ...
                            <?php foreach($config['lang'] as $key => $value) { ?>
                                <div id="tabs-<?=$key?>">
                                    ...
                                    <?php if($config['album'][$type]['mota']=='true') { ?>
                                        ...
                                        <?php if($config['album'][$type]['mota_cke']=='true') { ?>
                                            <script type="text/javascript">
                                                var mota<?=$key?> = CKEDITOR.replace('mota<?=$key?>');
                                            </script>
                                        <?php } ?>
                                    <?php } ?>
                                    <?php if($config['album'][$type]['noidung']=='true') { ?>
                                        ...
                                        <?php if($config['album'][$type]['noidung_cke']=='true') { ?>
                                            <script type="text/javascript">
                        ...
                        <?php if($config['album'][$type]['seo']=='true') { ?>
                        <!-- Thông Tin SEO -->
						...
						<p class="stdformbutton">
							...
							<button type="button" onclick="javascript:window.location='index.php?com=album&act=man&type=<?=$_GET['type']?>'" class="btn"><span class="iconfa-off"></span> Thoát</button>
						</p>	
					</form>
				</div>
			</div>
        ...
        jQuery(".delete_item_pic").click(function(event) {
            ...
            if (r == true) {
                ...
                jQuery.ajax({
                    ...
                    data: {'id':id,'tbl':'album_hinhanh','const':'photo'},
