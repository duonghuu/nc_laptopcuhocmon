	...
	<div class="contentinner">
		<div class="row-fluid">
			<div class="span12">
				...
				<form action="index.php?com=tinhthanh_quanhuyen&act=man_item" method="POST" name="update_position">
					<table class="table table-bordered" <?=(count($items))?'id="dyntable"':''?>>
						...
						<tr><input type="hidden" name="stt[id][]" value="<?=$items[$i]["id"]?>">
							...
							<td style="width:5%;"><input type="number" min="0" name="stt[stt][]" id="stt" value="<?=$items[$i]['stt']?>" data-id="<?=$items[$i]['id']?>" data-table="phuongxa" class="selectinput update_stt" /></td>
							<td style="width:30%;" class="text-left">
								<a href="index.php?com=tinhthanh_quanhuyen&act=edit_item&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id=<?=$items[$i]['id']?>" title="Chỉnh sửa"><?=$items[$i]['ten']?></a>
							</td>
							<td style="width:10%;" class="action">
								<a data-table="phuongxa" data-id="<?=$items[$i]['id']?>" data-loai="hienthi" class="show_ajax" title="Hiển thị">
							...
							<td style="width:10%;" class="action">
								<a href="index.php?com=tinhthanh_quanhuyen&act=edit_item&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id=<?=$items[$i]['id']?>" title="Chỉnh sửa"><span class="iconfa-edit"></span></a>&nbsp&nbsp
								<a href="index.php?com=tinhthanh_quanhuyen&act=delete_item&id=<?=$items[$i]['id']?>" title="Xóa" onClick="if(!confirm('Bạn có chắc muốn xóa mục này?')) return false;"><span class="iconfa-trash"></span></a>
							</td>
						</tr>
				...
				<div class="actionfull">
					<a href="index.php?com=tinhthanh_quanhuyen&act=add_item" class="btn btn-rounded"><icon class="icon-plus"></icon> Tạo mới</a>
...
<script>
	...
	function onSearch(evt){	
		var keyword = document.getElementById("keyword").value;		
			location.href = "index.php?com=tinhthanh_quanhuyen&act=man_item&keyword="+keyword;
	...
	if(jQuery('#delall').length > 0) {
		jQuery('#delall').click(function(){
			...
			if (hoi==true) document.location = "index.php?com=tinhthanh_quanhuyen&act=delete_item&listid=" + listid;
		});
	}
	...
	function select_onchange_1()
	{
		...
		var url = "index.php?com=tinhthanh_quanhuyen&act=man_item";
