...
<div class="breadcrumbwidget">
    <ul class="breadcrumb">
        ...
        <li class="active">Chi tiết <?=$config['lienket'][$type]['title_main']?></li>
    </ul>
</div>
<!-- End Bảng Điều Khiển -->
        ...
        <div class="row-fluid">
            <div class="span12">
                ...
                <div class="widgetcontent bordered shadowed nopadding">
                    <form name="frm" method="post" action="index.php?com=lienket&act=save&type=<?=$_GET['type']?>" enctype="multipart/form-data" class="stdform stdform2">
                        
                        <?php if($config['lienket'][$type]['images']=='true') { ?>
                            ...
                            <?php } ?>
                            <p><label>Upload hình ảnh:</label><span class="field"><input type="file" name="file" class="uniform-file" /> <strong><?php echo $config['lienket'][$type]['img_type']." - Width: ".$config['lienket'][$type]['width']." px - Height: ".$config['lienket'][$type]['height']." px"?></strong></span></p>
                        <?php } ?>

                        <p><label>Link:</label><span class="field"><input type="text" name="link" value="<?=@$item['link']?>" class="input-xxlarge" placeholder="Link *" required/></span></p>
                        
                        <?php if(($config['lienket'][$type]['tieude']!=false) || ($config['lienket'][$type]['mota']!=false) || ($config['lienket'][$type]['noidung']!=false)) { ?>
                            <!-- Begin Tab -->
                            <div id="tabs">
                                <ul>
                                ...
                                <?php foreach($config['lang'] as $key => $value) { ?>
                                    <div id="tabs-<?=$key?>">
                                        ...
                                        <?php if($config['lienket'][$type]['mota']=='true') { ?>
                                            ...
                                            <?php if($config['lienket'][$type]['mota_cke']=='true') { ?>
                                                <script type="text/javascript">
                                                    var mota<?=$key?> = CKEDITOR.replace('mota<?=$key?>');
                                                </script>
                                            <?php } ?>
                                        <?php } ?>
                                        <?php if($config['lienket'][$type]['noidung']=='true') { ?>
                                            ...
                                            <?php if($config['lienket'][$type]['noidung_cke']=='true') { ?>
                                                <script type="text/javascript">
                        ...
                        <?php if($config['lienket'][$type]['seo']=='true') { ?>
                        <!-- Thông Tin SEO -->
                        ...
                        <p class="stdformbutton">
                            ...
                            <button type="button" onclick="javascript:window.location='index.php?com=lienket&act=man&type=<?=$_GET['type']?>'" class="btn"><span class="iconfa-off"></span> Thoát</button>
                        </p>    
                    </form>
                </div>
            </div>
