...
<div class="breadcrumbwidget">
    <ul class="breadcrumb">
        ...
        <li class="active">Chi tiết <?=$config['news'][$type]['title_main_cat']?></li>
    </ul>
</div>
<!-- End Bảng Điều Khiển -->
		...
		<div class="row-fluid">
			<div class="span12">
				...
				<div class="widgetcontent bordered shadowed nopadding">
					<form name="frm" method="post" action="index.php?com=news&act=save_cat&type=<?=$_GET['type']?>" enctype="multipart/form-data" class="stdform stdform2">
                        ...
                        <?php if($config['news'][$type]['images_cat']=='true') { ?>
                            ...
                            <?php } ?>
                            <p><label>Upload hình ảnh:</label><span class="field"><input type="file" name="file" class="uniform-file" /> <strong><?php echo $config['news'][$type]['img_type_cat']." - Width: ".$config['news'][$type]['width_cat']." px - Height: ".$config['news'][$type]['height_cat']." px"?></strong></span></p>
                        <?php } ?>
                        ...
                        <div id="tabs">
                            ...
                            <?php foreach($config['lang'] as $key => $value) { ?>
                                <div id="tabs-<?=$key?>">
                                    ...
                                    <?php if($config['news'][$type]['mota_cat']=='true') { ?>
                                        ...
                                        <?php if($config['news'][$type]['mota_cke_cat']=='true') { ?>
                                            <script type="text/javascript">
                                                var mota<?=$key?> = CKEDITOR.replace('mota<?=$key?>');
                                            </script>
                                        <?php } ?>
                                    <?php } ?>
                                    <?php if($config['news'][$type]['noidung_cat']=='true') { ?>
                                        ...
                                        <?php if($config['news'][$type]['noidung_cke_cat']=='true') { ?>
                                            <script type="text/javascript">
                        ...
                        <?php if($config['news'][$type]['seo_cat']=='true') { ?>
                        <!-- Thông Tin SEO -->
						...
						<p class="stdformbutton">
							...
							<button type="button" onclick="javascript:window.location='index.php?com=news&act=man_cat&type=<?=$_GET['type']?>'" class="btn"><span class="iconfa-off"></span> Thoát</button>
						</p>	
					</form>
				</div>
			</div>
