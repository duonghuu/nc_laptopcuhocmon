...
<div class="breadcrumbwidget">
    <ul class="breadcrumb">
        ...
        <li class="active">Danh sách <?=$config['news_static'][$type]['title_main_photo']?></li>
    </ul>
</div>
<!-- End Bảng Điều Khiển -->

<div class="pagetitle">
	<h1>Danh sách <?=$config['news_static'][$type]['title_main_photo']?></h1>
</div>
...
<div class="maincontent">
	<div class="contentinner">
		<div class="row-fluid">
			<div class="span12">
				<form action="index.php?com=news_static&act=man_photo&type=<?=$_GET['type']?>" method="POST" name="update_position">
					<table class="table table-bordered" <?=(count($items))?'id="dyntable"':''?>>
						<thead>
							<tr>
						        ...
						        <?php if($config['news_static'][$type]['mausac_photo']=='true') { ?>
						        	<th>Màu sắc</th>
						        <?php } ?>
						        <?php if($config['news_static'][$type]['avatar_photo']=='true') { ?>
						        	<th>Hình ảnh</th>
						        <?php } ?>
							  	<th>Hiển thị</th>
								<th>Thao tác</th>
						...
						<tr><input type="hidden" name="stt[id][]" value="<?=$items[$i]["id"]?>">
							...
							<td style="width:5%;"><input type="number" min="0" name="stt[stt][]" id="stt" value="<?=$items[$i]['stt']?>" data-id="<?=$items[$i]['id']?>" data-table="news_static_hinhanh" class="selectinput update_stt" /></td>
							<?php if($config['news_static'][$type]['tieude_photo']=='true') { ?>
								<td style="width:30%;" class="text-left">
									<a href="index.php?com=news_static&act=edit_photo&id=<?=$items[$i]['id']?>&type=<?=$_GET['type']?>" title="Cập nhật"><?=$items[$i]['tenvi']?></a>
								</td>
							<?php } ?>
							<?php if($config['news_static'][$type]['mausac_photo']=='true') { ?>
								<td style="width:30%;">
							...
							<?php if($config['news_static'][$type]['avatar_photo']=='true') { ?>
								<td style="width:30%;" class="text-left">
									<a href="index.php?com=news_static&act=edit_photo&id=<?=$items[$i]['id']?>&type=<?=$_GET['type']?>" title="Cập nhật">
							...
							<td style="width:5%;" class="action">
								<a data-table="news_static_hinhanh" data-id="<?=$items[$i]['id']?>" data-loai="hienthi" class="show_ajax" title="Hiển thị">
							...
							<td style="width:10%;" class="action">
								<a href="index.php?com=news_static&act=edit_photo&id=<?=$items[$i]['id']?>&type=<?=$_GET['type']?>" title="Cập nhật"><span class="iconfa-edit"></span></a>&nbsp
								<a href="index.php?com=news_static&act=delete_photo&id=<?=$items[$i]['id']?>&type=<?=$_GET['type']?>" onClick="if(!confirm('Bạn có chắc muốn xóa dữ liệu ?')) return false;" title="Delete"><span class="iconfa-trash"></span></a>
							</td>
						</tr>
				...
				<div class="actionfull">
					<a href="index.php?com=news_static&act=add_photo&type=<?=$_GET['type']?>" class="btn btn-rounded"><icon class="icon-plus"></icon> Thêm mới</a>
					...
					<a href="index.php?com=news_static&act=capnhat&type=<?=$_GET['type']?>" class="btn btn-rounded"><icon class="icon-arrow-left"></icon> Quay về</a>
				</div>
			</div>
		</div>
	</div>
...
<script>
	...
	if(jQuery('#delall').length > 0) {
		jQuery('#delall').click(function(){
			...
			if (hoi==true) document.location = "index.php?com=news_static&act=delete_photo&type=<?=$_GET['type']?>&listid=" + listid;
		});
