<?php
    ...
    if(isset($_GET['src'])) 
    {
        if($dongdau['photo']!='')
        {
            $wmi = 'upload/photo/'.$dongdau['photo'];
        }
        else
        {
            $wmi = "assets/images/watermark.png";
        }
        if(!is_dir('nina@#cache/'))
        {
            mkdir('nina@#cache/',0755);
        }
        $imge = $img = 'nina@#cache/'.uniqid().".".strtolower(pathinfo( $_GET['src'], PATHINFO_EXTENSION ));
        $bas = $config_url_http;
